...
from cg3_steps import cg3_downstream_dataset, cg3_task_preprocess, cg3_protein, \
    cg3_graphconstruct

# if use GBT as the decoder based on the activation of 'gbt_use' function, set whether_emb_save to True to save embeddings of all involved samples into local
...
def train(cfg, model, optimizer, scheduler, train_set, valid_set, test_set, device, model_save_name, task_path, early_stop=None):
    best_epoch, best_val, early_stop_counter = None, -1e9, 0 # current best_val is for 'eval_metric: pearsonr [binding_affinity]'
...
def GBT_loop(cfg, encoder, decoder, train_set, test_set, normalization, dataset_name, device):
    ...
    if whether_emb_save == True:
        ...
        # change the save path into your local drive
...
if __name__ == "__main__":
    ...
    for fold in range(len(split_list)):
        ...
        if device.type == "cuda":
            # * the 'cuda' function used here comes from torchdrug/utils/torch.py, *
            # * in which the 'cuda' function in 'PackProtein' class is further called *
    ...
    print('\naverage RMSE, MAE, and Pearson on test set:', np.mean(RMSE_total), np.mean(MAE_total), np.mean(PEARSON_total))
    ...
    if gbt_pars == None:
        print('\nmodel save name for the last fold:', model_save_name)
