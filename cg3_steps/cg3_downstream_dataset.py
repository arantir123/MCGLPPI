...
from cg3_steps import cg3_protein
...
class PDBBINDDataset(data.ProteinDataset, PPIDataset):
    ...
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name='cg3_pdbbind.pkl.gz', transform=None, AA_num_threshold=5000,
                 raw_label_col='neg_log', label_upper=None, label_lower=None, cropping_threshold=None, contact_threshold=8, ancestor_tag=False,
    ...
    def load_cgs(self, cg_files, transform=None, AA_num_threshold=3000, verbose=0, **kwargs): # kwarg: {}
        ...
        for i, cg_file in enumerate(cg_files):
            ...
            # construct the protein class based on original force field files
            complete_check, protein = cg3_protein.CG3_Protein.from_cg_molecule(cg_file, AA_num_threshold=AA_num_threshold)
            ...
            # print(protein, protein.label, protein.extra_label) # CG3_Protein(num_atom=2414, num_bond=5164, num_residue=1019), tensor(0.7570), tensor(-9.2120)
            # print(hasattr(protein, 'label'), hasattr(protein, 'extra_label')) # True, True

            self.data.append(protein) # storing Protein class
            self.pdb_files.append(cg_file) # original cg file local locations: /cg_demo_martini3/1brs
...
class MANYDCDataset(PDBBINDDataset):
    ...
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name='cg3_manydc.pkl.gz', transform=None, AA_num_threshold=5000, raw_label_col='label',
...
class ATLASDataset(PDBBINDDataset):
    # when the inherited class contains initialization function, the 'super' needs to be used to initialize the ancestors
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name='cg3_atlas.pkl.gz', transform=None, AA_num_threshold=5000, raw_label_col='dG_update',
    ...
    def load_cgs(self, cg_files, transform=None, AA_num_threshold=3000, verbose=0, **kwargs): # kwarg: {}
        ...
        for i, cg_file in enumerate(cg_files):
            ...
            self.data.append(protein) # storing Protein class
            self.pdb_files.append(cg_file) # original cg file local locations: /cg_demo_martin3/1brs
            self.sequences.append(protein.aa_sequence if protein else None) # storing str protein sequences
    ...
    def create_labels(self, label_path, label_type='dG_update', extra_label_type='kd_update'):
        ...
        # * the 'create_labels' function will affect the label allocation in 'load_cgs' function *
