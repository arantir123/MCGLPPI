...
from cg3_steps import cg3_pretraining_dataset, cg3_task_preprocess, cg3_protein, \
    cg3_graphconstruct
...
if __name__ == "__main__":
    ...
    # while a major difference here is currently no validation and test sets are used, and thus no best_epoch is recorded
    # therefore the model storage logic could be little different, and the validation can also be added later

    t0 = time.time()
    # example: step=5, epoch=25 in total
    for i in range(0, cfg.train.num_epoch, step):
        ...
        if cfg.task['class'] == 'CGDiff':
            save(solver, "martini3_cgdiff_seed{}_gamma{}_bs{}_epoch{}_dim{}_length{}_radius{}_extra_{}.pth".format(args.seed, cfg.task.gamma, cfg.engine.batch_size, cfg.train.num_epoch,
            ...
            save(solver, "martini3_pretrain_seed{}_bs{}_epoch{}_dim{}_length{}_radius{}_extra_{}.pth".format(args.seed, cfg.engine.batch_size, cfg.train.num_epoch,
                cfg.task.model.embedding_dim, cfg.dataset.transform.transforms[0]['max_length'], cfg.task.graph_construction_model.edge_layers[0]['radius'], cfg.extra),
                cfg.get("save_model", True))
