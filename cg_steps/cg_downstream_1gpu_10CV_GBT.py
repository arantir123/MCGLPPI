...
from cg_steps import cg_downstream_dataset, cg_edgetransform, cg_graphconstruct, \
...
def train(cfg, model, optimizer, scheduler, train_set, valid_set, test_set, device, model_save_name, task_path, early_stop=None):
    ...
    for epoch in range(cfg.num_epoch):
        ...
        # * current evaluation set is same to the training set *
        model.eval()
        ...
        if cfg.model_save_mode == 'val':
            ...
            if metric[cfg.eval_metric] > best_val:
                ...
                if early_stop and early_stop == early_stop_counter:
                    # the specified evaluation metric results on best_epoch (determined based on validation set) for evaluation sets
