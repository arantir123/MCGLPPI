...
class CG22_Protein(Molecule):
    ...
    def clone(self):
        ...
        return type(self)(self.edge_list.clone(), bead_type=self.atom_type.clone(), bead2residue=self.bead2residue.clone(), node_position=self.node_position.clone(),
               ...
               backbone_angles=self.backbone_angles.clone(), backbone_sidec_angles=self.backbone_sidec_angles.clone(), sidechain_angles=self.sidechain_angles.clone(), backbone_dihedrals=self.backbone_dihedrals.clone())
    ...
    def residue_mask(self, index, compact=False):
        ...
        # edge_list has already been based on new relative node ids after above mapping function, num_node is also updated here
...
class CG22_PackedProtein(PackedMolecule, CG22_Protein):
    ...
    def __init__(self, edge_list=None, atom_type=None, bond_type=None, residue_type=None, view=None, num_nodes=None,
        ...
        # thus, current CG22_Protein will receive the angle information of the whole batch now (but not re-numbered for the whole batch yet)
