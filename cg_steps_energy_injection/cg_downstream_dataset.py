...
class MANYDCDataset(PDBBINDDataset):
    ...
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name='cg_manydc.pkl.gz', transform=None, AA_num_threshold=5000, raw_label_col='label',
        ...
        # data_path should be the path storing the original CG files
        data_path = os.path.expanduser(data_path)
        if not os.path.exists(data_path):
            os.makedirs(data_path)
...
class ATLASDataset(PDBBINDDataset):
    ...
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name='cg_atlas.pkl.gz', transform=None, AA_num_threshold=5000, raw_label_col='dG_update',
        ...
        if os.path.exists(pkl_file):
            ...
            # saving: sample number, storage path of original cg samples, protein sequences, cg protein classes
            self.save_pickle(pkl_file, verbose=verbose)
        # print(self.pdb_files[0], self.data[0].num_node) # atlas_data_subset\MT-1AO7_A69G_A_W101A_A_WT tensor(924)
    ...
    def create_labels(self, label_path, label_type='dG_update', extra_label_type='kd_update'):
        ...
        # ** extra update: potentially transform kd values into pkd values for better predictions **
        # ** create_labels will affect the label allocation in 'load_cgs' function **
        # extra_labels = -np.log10(extra_labels.astype(float))
        # print(np.max(extra_labels), np.min(extra_labels), np.mean(extra_labels)) # 4.5850, -3.5185, -1.0339
...
# Note: base class for inputting a pair of two-instance interacting complexes and outputting its ddG score (regression task, loss: MSE, metrics: MSE, MAE, Pearsonr)
@R.register("datasets.M1101Dataset")
class M1101Dataset(data.ProteinDataset, PPIDataset):
    # data_path: source data path, output_path: output pickle path, index_path: data splitting path
    def __init__(self, data_path, output_path, index_path, label_path, pickle_name="cg_m1101.pkl.gz", transform=None, AA_num_threshold=5000,
                 raw_label_col="ddG(kcal/mol)", label_upper=None, label_lower=None, cropping_threshold=None, contact_threshold=8,
                 ancestor_tag=False, task_tag="binding_affinity_change", verbose=1, **kwargs):
        ...
        if not ancestor_tag:
            ...
            self.label_dict = self.create_labels(label_path, raw_label_col, label_upper, label_lower) # label_type: ddG
            # read the dataset (for current fold)
            with open(index_path) as f:
                self.split_list = json.load(f)
            ...
            if os.path.exists(pkl_file):
                ...
                # currently AA_num_threshold is imposed to load_cgs such that the generated pickle read by load_pickle also satisfies the AA_num_threshold restriction
                self.load_cgs(cg_files, transform=transform, AA_num_threshold=AA_num_threshold, verbose=verbose, **kwargs)
                # saving: sample number, storage path of original cg samples, protein sequences, cg protein classes
                self.save_pickle(pkl_file, verbose=verbose)

            # * start to generate data splitting indicator based on the passed index file *
            # adjust the label category names from original splitting: https://github.com/arantir123/MpbPPI/blob/main/data/M1101_complex_data_split_256.jsonl
            self.split_list["test"] = self.split_list["val"]
            self.split_list["val"] = self.split_list["train"] # use training set as current validation set

            split_list_size, pdb_files_size = [len(self.split_list[i]) for i in self.split_list.keys()], len(self.pdb_files)
            # print(split_list_size, pdb_files_size) # [928, 928, 171] 1099
    ...
    def load_cgs(self, cg_files, transform=None, AA_num_threshold=3000, verbose=0, **kwargs): # kwarg: {}
        ...
        for i, cg_file in enumerate(cg_files):
            ...
            # need to construct CG graphs for WT and MT structures separately
            # cg_file example: /M1101_source/M1101_mt_wt_data/1-1AK4__1
            # print(os.path.split(cg_file)[0]) # /M1101_source/M1101_data_full
            wt_mt_check = True # ignore current sample point if either WT or MT structures cannot pass the completeness check
            for wt_mt_status in ["wt", "mt"]:
                # cg_file -> cg_subfile (to indicate 'WT' or 'MT' status)
                cg_subfile = os.path.join(cg_file, wt_mt_status)
                complete_check, protein = cg_protein.CG22_Protein.from_cg_molecule(cg_subfile, AA_num_threshold=AA_num_threshold)

                if not complete_check: # not passing the complete check (currently mainly for over-large protein check)
                    if isinstance(protein, str):
                        wt_mt_check = False
                        logger.debug("Can't construct protein from the CG file `%s`. Ignore this sample." % cg_subfile)
                        self.must_incomplete.append(cg_subfile)
                        continue
                    else: # for the case that the protein class is created successfully but the corresponding CG info may be incomplete
                        self.maybe_incomplete.append(cg_subfile)
                ...
                # * adding labels to current generated protein object *
                # * under current setting, the labels are stored in WT protein objects rather than MT ones *
                if wt_mt_status == "wt":
                    with protein.graph():
                        cg_file_key = os.path.normpath(cg_subfile).split(os.sep)[-2] # 1AK4__1
                        protein.label = torch.tensor(self.label_dict[cg_file_key])
                    # print(protein, protein.label)
                    # CG22_Protein(num_atom=671, num_bond=1528, num_residue=310), tensor(0.5700, dtype=torch.float64)
                if wt_mt_status == "wt":
                    wt = copy.deepcopy(protein)
                    wt_cg_subfile = cg_subfile
                else:
                    mt = copy.deepcopy(protein)
                    mt_cg_subfile = cg_subfile

            # store the WT-MT pairs for each sample point
            if wt_mt_check:
                self.data.append((wt, mt)) # storing pairwise protein class
                # [(CG22_Protein(num_atom=671, num_bond=1528, num_residue=310), CG22_Protein(num_atom=670, num_bond=1524, num_residue=310))]
                self.pdb_files.append(cg_file) # original cg file root locations for both WT and MT structures
                self.sequences.append((wt.aa_sequence if wt else None, mt.aa_sequence if mt else None)) # storing str protein sequences
    ...
    # * need to support the protein cropping for further decreasing computational cost and extracting core regions *
    def get_item(self, index):
        protein_name = os.path.basename(self.pdb_files[index]) # 1AK4__1

        # * label needs to be cloned from original WT protein independently *
        protein_label = self.data[index][0].label.clone()

        # print(self.data[index]) # (CG22_Protein(num_atom=671, num_bond=1528, num_residue=310), CG22_Protein(num_atom=670, num_bond=1524, num_residue=310))
        # * original 'clone' function for a protein is located in the basic 'graph' class, we re-write it in 'cg_protein' class *
        # * clone a protein object, transform function will be performed on it later (change view and crop graph for this clone) *
        wt = self.data[index][0].clone()
        mt = self.data[index][1].clone()

        # * earlier than 'transform' functions, as after one time of protein 'subgraph' function, the attribute 'aa_sequence' is not retained *
        if self.cropping_threshold:
            # cropping WT structures
            wt, closest_contact_distance = wt.protein_cropping(self.cropping_threshold, contact_threshold=self.contact_threshold)
            current_res_num = wt.residue_type.size(0)
            # print("retained residue and bead numbers of WT protein {} after protein cropping: {} and {}".
            #       format(protein_name, current_res_num, wt.num_node))

            assert current_res_num > 0, "WT protein {} is empty after the cropping with contact_threshold = {}, closest contact distance = {}".\
                format(protein_name, self.contact_threshold, closest_contact_distance)

            # cropping MT structures
            mt, closest_contact_distance = mt.protein_cropping(self.cropping_threshold, contact_threshold=self.contact_threshold)
            current_res_num = mt.residue_type.size(0)

            assert current_res_num > 0, "MT protein {} is empty after the cropping with contact_threshold = {}, closest contact distance = {}".\
                format(protein_name, self.contact_threshold, closest_contact_distance)

        if hasattr(wt, 'residue_feature'): # default: False
            with wt.residue():
    ...
    def create_labels(self, label_path, label_type="ddG(kcal/mol)", label_upper=None, label_lower=None):
        raw_labels = pd.read_csv(label_path)
        # * current pdb_names records the unique identity information for each sample point *
        pdb_names, resolutions, refined_labels = raw_labels["#ID"], raw_labels["PDB Res. (Angstroms)"], raw_labels[label_type].to_numpy()
        ...
        label_dict = dict()
        for pdb, label in zip(pdb_names, refined_labels):
            # current sample point name: 1-1AK4__1
            pdb = pdb.split("-")[-1]
            label_dict[pdb] = label
    ...
    def data_split(self):
        ...
        # * this order will be kept within each subset after the data splitting *
        for pdb_file, structure, sequence in zip(self.pdb_files, self.data, self.sequences):
            # pdb_file_ = os.path.basename(pdb_file) # 1AK4__1, used in local
            pdb_file_ = os.path.basename(pdb_file).split('\\')[-1] # used in server
    ...
    def tasks(self):
        """List of tasks."""
        return ["binding_affinity_change"]
