...
def cuda(obj, *args, **kwargs): # kwargs: device=0
    """
    Transfer any nested container of tensors to CUDA.
    """
    # the initial input (need to make its contained tensor go into cuda):
    # {'graph': CG22_PackedProtein(batch_size=4, num_atoms=[231, 120, 226, 120], num_bonds=[524, 244, 486, 244]),
    # 'graph2': CG22_PackedProtein(batch_size=4, num_atoms=[231, 120, 226, 120], num_bonds=[524, 244, 486, 244])}
    if hasattr(obj, "cuda"): # current obj has the attribute of 'cuda'
        # print(1, obj)
        return obj.cuda(*args, **kwargs) # enter the cuda function of PackProtein to put its tensors into cuda
    elif isinstance(obj, (str, bytes)):
        # print(2, obj)
        return obj
    elif isinstance(obj, dict): # current obj is a dict
...
def clone(obj, *args, **kwargs):
    """
    Clone tensors in any nested container.
    """
