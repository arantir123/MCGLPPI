...
class PropertyPrediction(tasks.Task, core.Configurable):
    ...
    def __init__(self, model, task=(), criterion="mse", metric=("mae", "rmse"), num_mlp_layer=1,
        ...
        self.model = model
        self.task = task
        self.criterion = criterion # criterion for generating loss
        self.metric = metric # evaluation metric
        self.num_mlp_layer = num_mlp_layer

        # for classification tasks, normalization tricks are disabled
        # it will be used for regression tasks with criteria like 'mse', for further normalizing the labels based on mean and std
    ...
    def preprocess(self, train_set, valid_set, test_set):
        ...
        for sample in train_set:
            ...
            # {'graph': CG22_Protein(num_atom=231, num_bond=524, num_residue=100), 'graph2': CG22_Protein(num_atom=231, num_bond=524, num_residue=100)}, False
        ...
        # print('values:', values)
        # values: defaultdict(<class 'list'>, {'binding_affinity': [tensor(1.6478, dtype=torch.float64), tensor(7.2218, dtype=torch.float64)]})
        ...
        # dict_items([]) (if task weights are empty), dict_items([('binding_affinity', 1)]) (if task is not empty)
        for task, w in self.task.items():
            ...
            # tensor([1.6478, 7.2218]), [tensor(4.4348)], [tensor(3.9414)], [1]
            if value.ndim > 1:
        ...
        # print(mean, std, weight, num_class) # [] [] [] [] (if task weights are empty), [tensor(4.4348)], [tensor(3.9414)], [1], [1] (if task is not empty)
        ...
        # print(self.mean, self.std, self.weight, self.num_class, self.mlp_batch_norm, self.mlp_dropout)
        # tensor([-6.7580]) tensor([1.8208]) tensor([1.]) (1,) True 0.2
