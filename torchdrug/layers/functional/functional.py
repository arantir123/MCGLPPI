...
# torch.Size([5198]), tensor([1916, 796, 1678, 808]), 256
def variadic_sample(input, size, num_sample):
    ...
    # print(rand, rand.size()) # torch.Size([4, 256]), randon values drawn from 0 to 1
    index = (rand * size.unsqueeze(-1)).long() # determine selected points for each protein
    # transform the relative index in each protein to the absolute index in current batch
    index = index + (size.cumsum(0) - size).unsqueeze(-1)
    sample = input[index]
    # print(sample, sample.size()) # torch.Size([4, 256])
    return sample
