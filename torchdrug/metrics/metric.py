...
def area_under_roc(pred, target):
    ...
    # print(pred, target)
    # tensor([-2.4121, -2.7760, 1.5659, ..., -2.8262, 0.3704, 0.3782])
    # tensor([0, 1, 1,  ..., 1, 0, 1])
    # print(pred.size(), target.size())
    # torch.Size([1148]) torch.Size([1148])

    # pred.argsort is not influenced by the scale of prediction (e.g., adding of sigmoid)
...
def accuracy(pred, target):
    ...
    # print(pred, pred.size(), pred.argmax(dim=-1), torch.sum(pred.argmax(dim=-1)))
