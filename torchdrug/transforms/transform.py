...
class TruncateProtein(core.Configurable):
    ...
    def __init__(self, max_length=None, random=False, keys="graph"):
        # default: max_length = 100, random = True
    ...
    def __call__(self, item):
        ...
        for key in self.keys:
            graph = item[key] # CG22_Protein(num_atom=439, num_bond=1014)

            if graph.num_residue > self.truncate_length:
                if self.random:
                    # making start + self.truncate_length will not exceed the full AA length of current protein
        ...
        # item, new_item: {'graph': CG22_Protein(num_atom=439, num_bond=1014)}, {'graph': CG22_Protein(num_atom=237, num_bond=544)}
        return new_item
...
class ProteinView(core.Configurable):
    ...
    def __call__(self, item):
        # self.view: residue
        # self.keys: ['graph']
        # item: {'graph': CG22_Protein(num_atom=439, num_bond=1014)}
        item = item.copy()
        for key in self.keys:
            graph = copy.copy(item[key])
            # print(graph.view) # atom
            graph.view = self.view
            item[key] = graph
            # print(graph.view) # residue
        return item
