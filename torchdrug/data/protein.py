...
class Protein(Molecule):
    ...
    # print(alphabet2id, id2residue, id2residue_symbol)
    # print(id2atom_name, id2alphabet)
    ...
    def __init__(self, edge_list=None, atom_type=None, bond_type=None, residue_type=None, view=None,
        ...
        self.view = self._standarize_view(view) # None
        ...
        with self.atom(): # assign atom information in the context of self.atom()
        ...
        with self.residue(): # assign residue information in the context of self.residue
    ...
    # this fucntion can be called using class_name.function_name()
    @classmethod
    ...
    def from_molecule(cls, mol, atom_feature="default", bond_feature="default", residue_feature="default",
        ...
        # print(mol, atom_feature, bond_feature, residue_feature, mol_feature, kekulize)
        # <rdkit.Chem.rdchem.Mol object at 0x00000290B577C2E0> default default default None False
        ...
        _residue_feature = []
        last_residue = None
        # 'atoms' contains the rdkit atom class for each atom to be parsed (in current rdkit protein)
        ...
        for atom in atoms:
            ...
            canonical_residue = (number, code, type) # AA serial number + insertion code + AA type
            if canonical_residue != last_residue:
                last_residue = canonical_residue
                if type not in cls.residue2id: # 20 standard residue types
                ...
                for name in residue_feature:
                    # one-hot embedding for the residue symbol
        ...
        # features returned for current protein class which could be important:
        # some of them may not be necessarily needed in CG protein class
        # print('protein.edge_list:', protein.edge_list) # standard torchdrug format
        # print('protein.num_node:', protein.num_node) # tensor(6562)
        # print('residue_type:', residue_type) # residue id tensor (1d)
        # print('atom_name:', atom_name) # atom_name: tensor([17,  1,  0,  ..., 15, 16, 34]), need to changed in CG class
        # print('atom2residue:', atom2residue) # atom2residue: tensor([  0,   0,   0,  ..., 846, 846, 846])
        # print('_residue_feature:', _residue_feature) # residue feature matrix (which may not used in CG class)
        # print('is_hetero_atom:', is_hetero_atom) # is_hetero_atom: tensor([False, False, False,  ..., False, False, False])
        # print('occupancy:', occupancy) # occupancy: tensor([1., 1., 1.,  ..., 1., 1., 1.])
        # print('b_factor:', b_factor) # b_factor: tensor([61.3900, 61.3900, 61.3900,  ..., 34.8300, 34.8300, 34.8300]), is_hetero_atom, occupancy, and b_factor and temporarily not needed
        # print('residue_number:', residue_number) # tensor([1, 2, 3, ..., 845, 846, 847]
        # print('insertion_code:', insertion_code) # all 0 tensor
        # print('chain_id:', chain_id) # all 1 tensor for current sample

        # print('protein.meta_dict:', protein.meta_dict)
        # protein.meta_dict: {'atom_feature': {'node'}, 'atom_type': {'node'}, 'formal_charge': {'node'}, 'explicit_hs': {'node'}, 'chiral_tag': {'node'}, 'radical_electrons': {'node'},
        # 'atom_map': {'node'}, 'node_position': {'node'}, 'bond_feature': {'edge'}, 'bond_type': {'edge'}, 'bond_stereo': {'edge'}, 'stereo_atoms': {'edge'}}

        # print('protein.data_dict:', protein.data_dict)
        # a dict that stores all information in protein.meta_dict
        # e.g., {''atom_type': tensor([7, 6, 6,  ..., 6, 6, 8])', 'bond_type': tensor([0, 0, 0,  ..., 3, 0, 0])}
        # based on this, maybe we can store all important features in protein.data_dict

        # print(protein.atom_feature, protein.node_feature, (protein.atom_feature-protein.node_feature).sum(), protein.atom_feature.size(), protein.node_feature.size())
        # tensor(0) torch.Size([6562, 67]) torch.Size([6562, 67]) # currenlty under atom view, protein.atom_feature and protein.node_feature are already the same
